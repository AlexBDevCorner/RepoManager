...
- System.Text.Json
- `git` through `System.Diagnostics.Process`
- xUnit for tests
...
- `Core` must not reference Avalonia or Infrastructure.
- Execute `git` directly — never through `cmd.exe` / `powershell.exe` / shells.
